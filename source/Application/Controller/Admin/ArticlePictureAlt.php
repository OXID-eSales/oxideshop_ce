...
class ArticlePictureAlt extends AdminDetailsController
{
    ...
    public function save(): void
    {
        ...
        foreach ($this->getSubmittedAltTexts($productMediaId) as $localeCode => $altText) {
            $altText !== ''
                ? $this->attributeService->save(MediaAttribute::ALT, $altText, $media, $localeCode)
                : $this->attributeService->delete(MediaAttribute::ALT, $media, $localeCode);
        }
    }
    ...
    private function buildImageData(ProductMedia $productMedia, array $locales): array
    {
        ...
        foreach ($locales as $locale) {
            ...
            $altTexts[$locale->getCode()] = $attributes->has(MediaAttribute::ALT) ? $attributes->getAlt() : '';
        }
