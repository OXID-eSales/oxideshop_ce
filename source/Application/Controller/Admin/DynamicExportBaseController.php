...
class DynamicExportBaseController extends \OxidEsales\Eshop\Application\Controller\Admin\AdminDetailsController
{
     ...
     */
    public function createMainExportView(): void
    {
     ...
     * Stops Export.
     *
     * @param int $iError error number
     */
    public function stop($iError = 0): void
    {
        if ($iError) {
     ...
     */
    public function setExportPerTick($iCount): void
    {
    ...
    public function removeSid($sInput)
    {
        ...
        return str_replace("sid={$sSid}", '', $sOutput);
    }
     ...
     *
     * @param string $sInput          input to process
     * @param int    $iMaxSize        maximum output size
     ...
     */
    // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    protected function _removeParentArticles($sHeapTable): void
    {
     ...
     */
    // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    protected function _setSessionParams(): void
    {
    ...
    protected function _findDeepestCatPath($oArticle)
    {
        ...
        if (\is_array($aIds) && \count($aIds)) {
            if ($aCatLvlCache = $this->_loadRootCats()) {
                ...
                // endless
                while (true) {
                    if (!isset($aCatLvlCache[$sIdMax]->oxparentid) || 'oxrootid' === $aCatLvlCache[$sIdMax]->oxparentid) {
                        break;
                    }
