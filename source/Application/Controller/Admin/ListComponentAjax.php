...
class ListComponentAjax extends \OxidEsales\Eshop\Core\Base
{
    ...
    protected function _getActionIds($sId)
    {
        ...
        foreach ($aColumns as $iPos => $aCol) {
            if (isset($aCol[4]) && 1 === $aCol[4] && $aCol[1] . '.' . $aCol[0] === $sId) {
    ...
    protected function _isExtendedColumn($sColumn)
    {
        ...
        return $this->_blAllowExtColumns && $blVariantsSelectionParameter && 'oxtitle' === $sColumn;
    }
    ...
    protected function _addFilter($sQ)
    {
        if ($sQ && ($sFilter = $this->_getFilter())) {
            $sQ .= (false === stristr($sQ, 'where') ? 'where' : ' and ') . $sFilter;
        }
    ...
    protected function _getSortDir()
    {
        ...
        if (!\in_array($sDir, $this->_aPosDir, true)) {
     ...
     */
    // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    protected function _outputResponse($aData): void
    {
     ...
     */
    // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    protected function _output($sOut): void
    {
        echo $sOut;
    }
     ...
     */
    // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    protected function _resetContentCache(): void
    {
    }
     ...
     */
    // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    protected function _resetCaches(): void
    {
    }
}
