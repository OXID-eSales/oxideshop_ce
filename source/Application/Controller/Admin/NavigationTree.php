...
class NavigationTree extends \oxSuperCfg
{
    ...
    protected function _addDynLinks($oDom)
    {
        ...
        foreach ($oNodeList as $oNode) {
            ...
            // fetching class
            $sClass = $oNode->getAttribute('cl');

            // always display the "about" tab no matter what licence

            if ($myUtilsFile->checkFile("{$this->_sDynIncludeUrl}pages/{$sClass}_about.php")) {
                $oTabElem = new DOMElement('TAB');
                $oNode->appendChild($oTabElem);
                $oTabElem->setAttribute('external', 'true');
                $oTabElem->setAttribute('location', "{$this->_sDynIncludeUrl}pages/{$sClass}_about.php");
                $oTabElem->setAttribute('id', 'dyn_about');
            }

            // checking for technics page
            if ($myUtilsFile->checkFile("{$this->_sDynIncludeUrl}pages/{$sClass}_technics.php")) {
                $oTabElem = new DOMElement('TAB');
                $oNode->appendChild($oTabElem);
                $oTabElem->setAttribute('external', 'true');
                $oTabElem->setAttribute('location', "{$this->_sDynIncludeUrl}pages/{$sClass}_technics.php");
            ...
            if (class_exists($sClass)) {
                ...
                $oNode->appendChild($oTabElem);
                $oTabElem->setAttribute('id', 'dyn_interface');
                $oTabElem->setAttribute('cl', $sClass);
            }
        }
    }
