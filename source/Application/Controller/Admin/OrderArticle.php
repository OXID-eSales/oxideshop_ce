...
class OrderArticle extends \OxidEsales\Eshop\Application\Controller\Admin\AdminDetailsController
{
    ...
    {
        if (null === $this->_oMainSearchProduct && ($sArtNum = $this->getSearchProductArtNr())) {
            ...
            if (false !== $rs && $rs->count() > 0) {
                $sArtId = $rs->fields['OXPARENTID'] ?: $rs->fields['OXID'];
    ...
    {
        if (null === $this->_oSearchProductList) {
     ...
     */
    public function addThisArticle(): void
    {
    ...
    public function storno(): void
    {
        ...
        if (1 === $oArticle->oxorderarticles__oxstorno->value) {
        ...
        $oDb->execute($sQ, [
            ':oxstorno' => $oArticle->oxorderarticles__oxstorno->value,
            ':oxid' => $sOrderArtId,
        ]);
