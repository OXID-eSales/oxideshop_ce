...
class SelectListMainAjax extends \OxidEsales\Eshop\Application\Controller\Admin\ListComponentAjax
{
    ...
    public function addArtToSel(): void
    {
        ...
        if ($soxId && '-1' !== $soxId && \is_array($aAddArticle)) {
            ...
            foreach ($aAddArticle as $sAdd) {
                ...
                $oNewGroup->oxobject2selectlist__oxsort = new \OxidEsales\Eshop\Core\Field((int)$database->getOne('select max(oxsort) + 1 from oxobject2selectlist where oxobjectid = :oxobjectid', [
                    ':oxobjectid' => $sAdd,
                ]));
                $oNewGroup->save();
     ...
     */
    protected function onArticleAddToSelectionList($articleId): void
    {
    }
}
