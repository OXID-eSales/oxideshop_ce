    ...
    public function render()
    {
        ...
        if (isset($confVars['arr']['aHomeCountry']) && \count($confVars['arr']['aHomeCountry']) && \count($countryList)) {
            foreach ($countryList as $sCountryId => $oCountry) {
                if (\in_array($oCountry->oxcountry__oxid->value, $confVars['arr']['aHomeCountry'], true)) {
            ...
            if (\is_array($requestValue)) {
                foreach ($requestValue as $configName => $newConfigValue) {
                    ...
                    if ($newConfigValue !== $oldValue) {
                        $sValueToValidate = \is_array($newConfigValue) ? implode(', ', $newConfigValue) : $newConfigValue;
    ...
    public function _unserializeConfVar($type, $name, $value)
    {
        ...
        switch ($type) {
            ...
            case 'int':
                ...
                if (\in_array($name, $this->_aParseFloat, true)) {
                    $data = str_replace(',', '.', $data);
                }
                break;

            case 'arr':
                if (\in_array($name, $this->_aSkipMultiline, true)) {
