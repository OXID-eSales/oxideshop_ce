...
class PictureHandler extends \OxidEsales\Eshop\Core\Base
{
    ...
    public function deleteArticleMasterPicture($oObject, $iIndex, $blDeleteMasterPicture = true): void
    {
        ...
        if (!$sMasterImage || 'nopic.jpg' === $sMasterImage) {
            return;
        }
        ...
        if ($oObject->{'oxarticles__oxzoom' . $iIndex}->value) {
            if ('nopic.jpg' !== basename($oObject->{'oxarticles__oxzoom' . $iIndex}->value)) {
    ...
    public function deleteZoomPicture($oObject, $iIndex): void
    {
        ...
        if ($sZoomPicName && 'nopic.jpg' !== $sZoomPicName) {
    ...
    public function getAltImageUrl($sFilePath, $sFile, $blSSL = null)
    {
        ...
        if ($sAltUrl) {
            if ((null === $blSSL && $oConfig->isSsl()) || $blSSL) {
