...
class oxUtilsDate extends oxSuperCfg
{
    ...
    {
        $sDate = date('Y-m-d H:i:s', $iTimestamp);
        return $sDate;
    }

    /**
     * Returns the the timestamp formatted as date string for the database
     * @param $iRoundTo a amount of seconds to be rounded to e.g. 300 for rounding to 5 minutes
     *
     * @return bool|string  the data string formatted for the database (SQL), false on error
     */
    public function getRoundedRequestDateDBFormatted($iRoundTo)
    {
        $iTimestamp = $this->getRequestTime();
        //round up x minutes so query cache can work
        $iTimestamp = ceil($iTimestamp / $iRoundTo) * $iRoundTo;
        //format date for sql query
        $sDate = $this->formatDBTimestamp($iTimestamp);
        return $sDate;
    }
