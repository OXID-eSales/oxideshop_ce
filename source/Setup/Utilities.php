...
class Utilities extends Core
{
    ...
    public const DEMODATA_PACKAGE_NAME = 'oxideshop-demodata-%s';
    ...
    {
        return md5(uniqid(random_int(0, getrandmax()), true));
    }
            ...
            while (false !== ($sEntry = $d->read())) {
                if ('.' !== $sEntry && '..' !== $sEntry) {
                    ...
                    if (is_file($sFilePath)) {
                        if (!\in_array(basename($sFilePath), $aSkipFiles, true)) {
                        ...
                        if (0 === $iMode && !\in_array(basename($sFilePath), $aSkipFolders, true)) {
     ...
     */
    public function updateConfigFile($parameters): void
    {
        $configFile = '';
     ...
     */
    private function handleMissingConfigFileException($pathToConfigFile): void
    {
     ...
     */
    public function updateHtaccessFile($aParams, $sSubFolder = ''): void
    {
        ...
        if (!file_exists($sHtaccessPath)) {
            throw new Exception(sprintf($oLang->getText('ERROR_COULD_NOT_FIND_FILE'), $sHtaccessPath), self::ERROR_COULD_NOT_FIND_FILE);
        }
        ...
        if (is_readable($sHtaccessPath) && ($fp = fopen($sHtaccessPath, 'r'))) {
            ...
            throw new Exception(sprintf($oLang->getText('ERROR_COULD_NOT_READ_FILE'), $sHtaccessPath), self::ERROR_COULD_NOT_READ_FILE);
        }
        ...
        if (is_writable($sHtaccessPath) && ($fp = fopen($sHtaccessPath, 'w'))) {
            ...
            // error ? strange !?
            throw new Exception(sprintf($oLang->getText('ERROR_COULD_NOT_WRITE_TO_FILE'), $sHtaccessPath), self::ERROR_COULD_NOT_WRITE_TO_FILE);
        }
    }
    ...
    {
        if (false !== ($sVarVal = getenv($sVarName))) {
            return $sVarVal;
        }
    }
     ...
     */
    public function setCookie($sName, $sValue, $iExpireDate, $sPath): void
    {
    ...
    {
        $sPath = '/';
        if (false !== ($aPathInfo = @parse_url($sUrl))) {
    ...
    {
        return 0 !== preg_match($this->_sEmailTpl, $sEmail);
    }
     ...
     */
    public function executeExternalDatabaseMigrationCommand(ConsoleOutput $output = null, Facts $facts = null): void
    {
