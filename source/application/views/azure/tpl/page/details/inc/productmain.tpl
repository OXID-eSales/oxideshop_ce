        ...
        <div class="productMainInfo[{if $oManufacturer->oxmanufacturers__oxicon->value}] hasBrand[{/if}]">
            ...
            [{block name="details_productmain_ratings"}]
                <div class="rating clear">
                    ...
                    [{/if}]
                    [{oxid_include_widget cl="oxwRating" blCanRate=$oView->canRate() _parent=$oViewConf->getTopActiveClassName() nocookie=1 force_sid=$force_sid sRateUrl=$oDetailsProduct->getLink() dRatingCount=$oView->getRatingCount() dRatingValue=$oView->getRatingValue() anid=$oDetailsProduct->oxarticles__oxnid->value skipreverceproxy=$oxcmp_user}]
                </div>
            [{/block}]
            [{/if}]
        </div>
