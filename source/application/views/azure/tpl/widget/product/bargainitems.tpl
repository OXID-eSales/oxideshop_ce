...
[{foreach from=$oView->getBargainArticleList() item=_product name=bargainList}]
    [{if $smarty.foreach.bargainList.first}]
        ...
        [{oxid_include_widget cl="oxwArticleBox" _parent=$oView->getClassName() nocookie=1 _navurlparams=$oViewConf->getNavUrlParams() iLinkType=$_product->getLinkType() object=$_product anid=$_product->getId() isVatIncluded=$oView->isVatIncluded() rsslinks=$rsslinks iIteration=$smarty.foreach.bargainList.iteration sWidgetType=product sListType=bargainitem inlist=$_product->isInList() skipESIforUser=1}]
    [{/if}]
[{/foreach}]
