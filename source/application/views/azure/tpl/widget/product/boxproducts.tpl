...
<div class="box" [{if $_boxId}]id="[{$_boxId}]"[{/if}]>
    ...
    [{foreach from=$_oBoxProducts item=_oBoxProduct name=_sProdList}]
            [{oxid_include_widget cl="oxwArticleBox" _navurlparams=$oViewConf->getNavUrlParams() sProductId=$_oBoxProduct->getId() nocookie=1 sWidgetType=product sListType=boxproduct}]
    [{/foreach}]
    </ul>
</div>