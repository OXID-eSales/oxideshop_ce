...
<div class="box" [{if $_boxId}]id="[{$_boxId}]"[{/if}]>
    ...
    [{foreach from=$_oBoxProducts item=_oBoxProduct name=_sProdList}]
            ...
            [{oxid_include_widget cl="oxwArticleBox" _parent=$oView->getClassName() _navurlparams=$oViewConf->getNavUrlParams() anid=$_oBoxProduct->getId() currencySign=$currency->sign isVatIncluded=$oView->isVatIncluded() iProdCount=$iProdCount nocookie=1 sWidgetType=product sListType=boxproduct}]
    [{/foreach}]
    </ul>
</div>