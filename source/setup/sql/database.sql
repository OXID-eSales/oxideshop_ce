...
CREATE  TABLE IF NOT EXISTS `oxvoucherseries` (
  ...
  `OXAPPLYONBRUTTO` TINYINT(1) UNSIGNED NOT NULL DEFAULT '0' COMMENT 'Calculate absolute discount after VAT is applied',
  `OXUSEBASKETPRICE` TINYINT(1) UNSIGNED NOT NULL DEFAULT '0' COMMENT 'Calculate discount basing on basket items prices',
  `OXDONTREDUCEPRICE` TINYINT(1) UNSIGNED NOT NULL DEFAULT '0' COMMENT 'Do not reduce base amount for next discounts after the voucher was applied',
  `OXTIMESTAMP` timestamp NOT NULL default CURRENT_TIMESTAMP on update CURRENT_TIMESTAMP COMMENT 'Timestamp',
  PRIMARY KEY  (`OXID`),
