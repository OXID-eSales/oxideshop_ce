...
class Account_Downloads extends Account
{
    ...
    public function getBreadCrumb()
    {
        ...
        $aPath['title'] = oxLang::getInstance()->translateString( 'MY_ACCOUNT', oxLang::getInstance()->getBaseLanguage(), false );
        $aPath['link']  =  oxSeoEncoder::getInstance()->getStaticUrl( $this->getViewConfig()->getSelfLink() . "cl=account" );
        $aPaths[] = $aPath;

        $aPath['title'] = oxLang::getInstance()->translateString( 'MY_DOWNLOADS', oxLang::getInstance()->getBaseLanguage(), false );
