...
class order extends oxUBase
{
    ...
    protected function _validateTermsAndConditions()
    {
        ...
        if ($oConfig->getConfigParam('blConfirmAGB') && !$oConfig->getParameter('ord_agb')) {
            $blValid = false;
        }
        if ($oConfig->getConfigParam('blEnableIntangibleProdAgreement')) {
            $oBasket = $this->getBasket();
            if ($blValid && $oBasket->hasArticlesWithDownloadableAgreement() && !$oConfig->getParameter('oxdownloadableproductsagreement')) {
                $blValid = false;
            }
        if ($blValid && $oBasket->hasArticlesWithIntangibleAgreement() && !$oConfig->getParameter('oxserviceproductsagreement')) {
                        $blValid = false;
                    }
        }
        return $blValid;
