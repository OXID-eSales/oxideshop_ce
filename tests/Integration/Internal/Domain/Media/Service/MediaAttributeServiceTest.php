...
final class MediaAttributeServiceTest extends IntegrationTestCase
{
    ...
    {
        $this->dao->save($this->createAttribute($this->media, $this->locale, 'alt', 'alt value'));
    ...
    {
        $this->service->save('alt', 'some alt text', $this->media, $this->locale->getCode());
    ...
    {
        $this->service->save('alt', 'original', $this->media, $this->locale->getCode());
        $this->service->save('alt', 'updated', $this->media, $this->locale->getCode());
    ...
    public function testDelete(): void
    {
        ...
        $this->service->delete('alt', $this->media, $this->locale->getCode());

        $this->assertFalse(
            $this->dao->getAttributes($this->media->getId(), new LocaleChain([$this->locale->getCode()]), $this->shopId)->has('alt')
        );
    }

    public function testSaveKeepsGivenValue(): void
    {
        $this->service->save('alt', '  stored  ', $this->media, $this->locale->getCode());
    ...
    public function testGetAttributesReturnsOnlyRequestedLocale(): void
    {
        ...
        $this->dao->save($this->createAttribute($this->media, $this->locale, 'alt', 'value from active locale'));
