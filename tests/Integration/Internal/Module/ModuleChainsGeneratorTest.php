 ...
 */
class ModuleChainsGeneratorTest extends IntegrationTestCase
{
    /**
    ...
    public function setupBCAutoloader()
    {
        $reflect = new \ReflectionClass(BackwardsCompatibilityAutoload::class);
        require_once $reflect->getFilename();
    ...
    public function testCreateClassChain($mockedModules, $modulesArray, $expectedResult, $message)
    {
        ...
        $moduleVariablesLocatorMock = $this->getMockBuilder(ModuleVariablesLocator::class)
         ...
         */
        $moduleChainsGeneratorMock = $this->getMockBuilder(ModuleChainsGenerator::class)
            ->setMethods(['getClassExtensionChain'])
            ->setConstructorArgs([$moduleVariablesLocatorMock])
            ->getMock();
