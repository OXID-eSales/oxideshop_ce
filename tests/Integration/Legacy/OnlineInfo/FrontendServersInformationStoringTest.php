...
class FrontendServersInformationStoringTest extends IntegrationTestCase
{
    ...
    private function getApplicationServerServiceObject($currentTime)
    {
        ...
        $utilsServer = $this->getMockBuilder(\OxidEsales\Eshop\Core\UtilsServer::class)
            ->getMock();
        $utilsServer->expects($this->any())->method('getServerNodeId')->willReturn('serverNameHash1');
        $utilsServer->expects($this->any())->method('getServerIp')->willReturn('127.0.0.1');
