 ...
 */
class ActionsMainAjaxTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testGetQuerySynchoxid()
    {
        ...
        $oView = oxNew('actions_main_ajax');
        $this->assertSame("from " . $this->getArticleViewTable() . " where 1  and " . $this->getArticleViewTable() . ".oxparentid = ''  and " . $this->getArticleViewTable() . sprintf(".oxid not in ( select oxactions2article.oxartid from oxactions2article  where oxactions2article.oxactionid = '%s' and oxactions2article.oxshopid = '", $sSynchoxid) . $this->getShopIdTest() . "' )", trim((string) $oView->getQuery()));
    }
    ...
    public function testGetQuerySynchoxidVariantsSelectionTrue()
    {
        ...
        $this->getConfig()->setConfigParam("blVariantsSelection", true);
        $oView = oxNew('actions_main_ajax');
        $this->assertSame("from " . $this->getArticleViewTable() . " where 1  and " . $this->getArticleViewTable() . sprintf(".oxid not in ( select oxactions2article.oxartid from oxactions2article  where oxactions2article.oxactionid = '%s' and oxactions2article.oxshopid = '", $sSynchoxid) . $this->getShopIdTest() . "' )", trim((string) $oView->getQuery()));
    }
    ...
    public function testGetQueryOxid()
    {
        ...
        $oView = oxNew('actions_main_ajax');
        $this->assertSame("from " . $this->getArticleViewTable() . " left join oxactions2article on " . $this->getArticleViewTable() . sprintf(".oxid=oxactions2article.oxartid  where oxactions2article.oxactionid = '%s' and oxactions2article.oxshopid = '", $sOxid) . $this->getShopIdTest() . "'", trim((string) $oView->getQuery()));
    }
    ...
    public function testGetSortingOxid()
    {
        ...
        $oView = oxNew('actions_main_ajax');
        $this->assertSame("order by oxactions2article.oxsort", trim((string) $oView->getSorting()));
    }
    ...
    public function testRemoveArtFromAct()
    {
        ...
        $this->assertSame(2, oxDb::getDb()->getOne("select count(oxid) from oxactions2article where oxactionid='_testActionAdd'"));
        $oView->removeartfromact();
        $this->assertSame(0, oxDb::getDb()->getOne("select count(oxid) from oxactions2article where oxactionid='_testActionAdd'"));
    }
    ...
    public function testAddArtToAct()
    {
        ...
        $this->assertSame(0, oxDb::getDb()->getOne(sprintf("select count(oxid) from oxactions2article where oxactionid='%s'", $sSynchoxid)));

        $oView = $this->getMock(\OxidEsales\Eshop\Application\Controller\Admin\ActionsMainAjax::class, ["getActionIds"]);
        $oView->method('getActionIds')->willReturn(['_testActionAdd1', '_testActionAdd2']);

        $oView->addarttoact();
        $this->assertSame(2, oxDb::getDb()->getOne(sprintf("select count(oxid) from oxactions2article where oxactionid='%s'", $sSynchoxid)));
    }
