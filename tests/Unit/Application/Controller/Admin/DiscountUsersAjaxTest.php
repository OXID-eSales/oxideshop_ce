...
class DiscountUsersAjaxTest extends \OxidTestCase
{
    ...
    {
        $oView = $this->getMock(\OxidEsales\Eshop\Application\Controller\Admin\DiscountUsersAjax::class, array("getActionIds"));
    ...
    public function testAddDiscUser()
    {
        ...
        $this->setRequestParameter("synchoxid", $sSynchoxid);
        $oView = $this->getMock(\OxidEsales\Eshop\Application\Controller\Admin\DiscountUsersAjax::class, array("getActionIds"));
        $oView->expects($this->any())->method('getActionIds')->will($this->returnValue(array('_testNewUser1', '_testNewUser2')));
        $this->assertEquals(3, oxDb::getDb()->getOne("select count(oxid) from oxobject2discount where oxdiscountid='_testDiscount'"));
