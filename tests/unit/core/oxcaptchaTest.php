...
class Unit_Core_oxcaptchaTest extends OxidTestCase
{
    ...
    {
        modConfig::getInstance()->setConfigParam('captchaKey', 'someTestCaptchaKey');

        $this->_oCaptcha->setNonPublicVar('_sText', 'test1');
        $this->assertEquals(modConfig::getInstance()->getShopUrl() . "core/utils/verificationimg.php?e_mac=ox_MAsbCBYgVBoQ", $this->_oCaptcha->getImageUrl());
    }
    ...
    {
        modConfig::getInstance()->setConfigParam('captchaKey', '');
